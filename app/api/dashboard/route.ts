...
import { getStatsAdmin, getAllActiveLoansWithPayments } from '@/lib/firestore/loans';
...
export async function GET() {
  try {
    ...
    // Single fetch — all active loans + their payments; filter everything in JS
    ...
    // Heatmap: last 90 days of paid activity, indexed by date
    ...
    // Cashflow: last 14 days — expected vs collected per day
    ...
    // Plan split — active loans
    let dailyLoanCount = 0;
    let weeklyLoanCount = 0;
    let dailyPrincipal = 0;
    let weeklyPrincipal = 0;
    ...
    for (const { loan, payments } of loansWithPayments) {
      ...
      for (const p of payments) {
        ...
        // Heatmap
        if (p.paidDate && p.paidDate >= heatmapStartStr && p.paidAmount > 0) {
          dailyCollected[p.paidDate] = (dailyCollected[p.paidDate] || 0) + p.paidAmount;
        }
        // Cashflow
        ...
        // This week expected vs collected
        if (p.dueDate >= today && p.dueDate <= weekEnd) {
    ...
    const recentActivity = recentPayments
      .sort((a: any, b: any) => b.paidDate > a.paidDate ? 1 : -1)
      .slice(0, 8);
    ...
    // Cashflow array (last 14 days up to today)
    ...
    // Top borrowers by outstanding (top 5)
    const topBorrowers = Object.values(borrowerOutstanding)
    ...
    return NextResponse.json({
      stats: {
        ...stats,
        total_collected: totalRecovered,
        total_expected_interest: stats.interest_pending,
      },
      overduePayments,
      dueSoon,
      monthlyData,
      ...
      cashflow,
      planSplit: {
        daily: { count: dailyLoanCount, principal: dailyPrincipal },
        weekly: { count: weeklyLoanCount, principal: weeklyPrincipal },
      },
      topBorrowers,
    });
  } catch (e) {
